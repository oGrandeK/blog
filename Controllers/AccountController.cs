...
public class AccountController : ControllerBase
{
    ...
    public async Task<IActionResult> Post([FromBody] RegisterViewModel model, [FromServices] EmailService emailService, [FromServices] BlogDataContext context)
    {
        ...
        {
            return StatusCode(400, new ResultViewModel<string>("05x99 - Este E-mail já está em cadastrado"));
        }
        catch
        {
    ...
    public async Task<IActionResult> Login([FromServices] BlogDataContext context, [FromServices] TokenService tokenService, [FromBody] LoginViewModel model)
    {
        if (!ModelState.IsValid) return BadRequest(new ResultViewModel<string>(ModelState.GetErrors()));
        ...
        if (!PasswordHasher.Verify(user.PasswordHash, model.Password)) return StatusCode(401, new ResultViewModel<string>("Usuário ou senha inválido"));
